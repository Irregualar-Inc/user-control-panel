...
def create_user_permissions():
    ...
    try:
        ...
        _create_user_permissions(user_doc.name, company, employee_id, cost_center)
        frappe.db.commit()
...
def _create_user_permissions(
    user_id: str, company: str, employee_id: str, cost_center: str
) -> None:
	"""
	...
	try:
		...
		# 		}
		# 	).save()
		# 	print("created company permission")
		frappe.db.commit()
		...
		if not frappe.db.exists("User Control Panel", {"employee": employee_id}):
			...
			restriction_masters = ["Employee", "Cost Center"]

			# Add restrictions from control panel settings to the User Control Panel
			for master in restriction_masters:
				user_control_panel.append(
					"restrictions",
					{
						"allow": master,
						"for_value": (employee_id if master == "Employee" else cost_center),
						"is_default": 0,
						"apply_to_all_doctypes": 1,
						"applicable_for": "",
						"hide_descendants": 0,
